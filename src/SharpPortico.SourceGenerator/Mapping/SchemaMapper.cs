...
internal sealed class SchemaMapper
{
    ...
    public IReadOnlyList<MessageModel> GetAllMessages() => _messages.Values.ToList();
    ...
    public MessageModel? MapSchemaToMessage(string name, OpenApiSchema schema, bool isRequest, bool isResponse)
    {
        ...
        if (schema.AllOf is { Count: 1 }
            && schema.AllOf[0].Reference is not null
        ...
        MapSchemaShape(fields, schema, ref fieldNo);
    ...
    private void MapSchemaShape(List<FieldModel> fields, OpenApiSchema schema, ref int fieldNo)
    {
        if (schema.AllOf is { Count: > 0 })
        {
            foreach (var sub in schema.AllOf)
            {
                MergeSchemaFields(this, fields, sub, ref fieldNo);
            }
            // merge own properties after allOf
            MergeSchemaProperties(this, fields, schema, ref fieldNo);
        }
        else if (schema.OneOf is { Count: > 0 } || schema.AnyOf is { Count: > 0 })
        {
            MergeCompositionFields(this, fields, schema, ref fieldNo);
        }
        else if (schema.Properties is { Count: > 0 } || schema.Type == ObjectSchemaType)
        {
            MergeSchemaProperties(this, fields, schema, ref fieldNo);
        }
        else if (schema.Type == "array")
        {
            ...
            var field = MapProperty(this, "Values", schema, ref fieldNo);
            if (field is not null) fields.Add(field);
        }
        else
        {
            // Scalar top-level -> wrap into Value field (rare for components/schemas)
            var scalar = MapProperty(this, "Value", schema, ref fieldNo);
            if (scalar is not null) fields.Add(scalar);
        }
    }

    private static void MergeCompositionFields(SchemaMapper owner, List<FieldModel> fields, OpenApiSchema schema, ref int fieldNo)
    {
        var choices = schema.OneOf is { Count: > 0 } ? schema.OneOf : schema.AnyOf;
        if (choices.Count == 1)
        {
            MergeSchemaFields(owner, fields, choices[0], ref fieldNo);
            return;
        }
        // Add a string discriminator + flatten the first object-valued choice
        fields.Add(new FieldModel("Kind", "kind", ++fieldNo, FieldKind.String, StringFieldType));
        var chosen = choices.FirstOrDefault(c => c.Properties is { Count: > 0 }) ?? choices[0];
        MergeSchemaFields(owner, fields, chosen, ref fieldNo);
    }

    /// <summary>
    /// Resolves the generated type name when the schema is a $ref; otherwise <c>null</c>.
    /// </summary>
    public static string? ResolveSchemaName(OpenApiSchema schema)
    ...
    /// </summary>
    public static (bool IsPagination, FieldModel? Field) MapParameter(
        SchemaMapper owner,
        ...
        ref int index)
    {
        ...
        var field = MapProperty(owner, parameter.Name, parameter.Schema, ref index);
    ...
    public static bool IsArrayOfInlineObject(OpenApiSchema schema)
        => schema.Type == "array"
           && schema.Items is not null
           && schema.Items.Reference is null
           && (schema.Items.Properties is { Count: > 0 } || schema.Items.Type == ObjectSchemaType);

    /// <summary>Derives a deterministic name for the element message of an inline array schema.</summary>
    public static string InlineArrayElementName(OpenApiSchema arraySchema, string fallbackBase)
    ...
    private static void MergeSchemaFields(SchemaMapper owner, List<FieldModel> fields, OpenApiSchema schema, ref int fieldNo)
    {
        owner._ct.ThrowIfCancellationRequested();
        MergeSchemaProperties(owner, fields, schema, ref fieldNo);

        if (schema.AllOf is { Count: > 0 })
        {
            foreach (var sub in schema.AllOf)
            {
                MergeSchemaFields(owner, fields, sub, ref fieldNo);
            }
        }
    ...
    private static void MergeSchemaProperties(SchemaMapper owner, List<FieldModel> fields, OpenApiSchema schema, ref int fieldNo)
    {
        ...
        foreach (var kvpProp in schema.Properties)
        {
            ...
            var field = MapProperty(owner, propName, propSchema, ref fieldNo);
            if (field is not null) fields.Add(field);
        }
    }
    ...
    private static FieldModel? MapProperty(SchemaMapper owner, string name, OpenApiSchema schema, ref int fieldNo)
    {
        ...
        if (schema.Reference is not null)
        {
            ...
            bool isEnum = owner.ComponentSchemas is not null
                && owner.ComponentSchemas.TryGetValue(schema.Reference.Id, out var target)
                && IsEnum(target);
        ...
        {
            return MapArrayProperty(owner, name, schema, ref fieldNo);
        }
        ...
        if (schema.Properties is { Count: > 0 } || schema.Type == ObjectSchemaType)
        {
            ...
            {
                owner.MapSchemaToMessage(nestedName, schema, isRequest: false, isResponse: false);
            }
    ...
    private static FieldModel? MapArrayProperty(SchemaMapper owner, string name, OpenApiSchema schema, ref int fieldNo)
    {
        ...
        if (items.Properties is { Count: > 0 } || items.Type == ObjectSchemaType)
        {
            ...
            if (!owner._messages.ContainsKey(nestedName))
            {
                owner.MapSchemaToMessage(nestedName, items, isRequest: false, isResponse: false);
            }
    ...
    private static FieldModel? MapScalarField(string name, OpenApiSchema schema, ref int fieldNo)
    {
        ...
        {
            FieldKind.String => StringFieldType,
            FieldKind.Int32 => "int",
